 ...
 * the user does not need to necessarily free them manually. It's not recommended that resources be free immediately
...
public interface Data<T> extends Iterable<T> {
     ...
     * @param flags Bit field containing flags containing extra information about this request for an element.
     ...
     * loading operations. Note that it's common for the data to be in a "hidden" state for a very short interval, such
     * as during a configuration change, so any cancelation should probably occur after a short delay. {@code 3}
     * seconds
     * should suffice.
     */
    void notifyHidden();
